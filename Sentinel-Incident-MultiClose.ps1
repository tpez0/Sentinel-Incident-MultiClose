...
$SeverityNum = $(Write-Host 'Enter Severity to close: [ 0-3 ] ' -ForegroundColor Yellow -NoNewline; Read-Host)
Clear-Host
...
# If selected Severity is HIGH, all available titles will be loaded in order to select a specific type of incident
if ($SeverityNum -eq '3'){
Write-Host "Loading available High Severity Incidents Titles" -ForegroundColor Magenta
    Write-Host "It takes a lot of time. Go get a coffee!" -ForegroundColor Red
...
if ($SeverityNum -eq '3'){Write-Host "      Selected High Severity incidents: " -NoNewline -ForegroundColor Green; $HighTitleList[$HighTitleNum]; Write-Host "      Classification Comment: " -NoNewline -ForegroundColor Green; $HighComment}
